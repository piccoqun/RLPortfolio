...
from pgportfolio.tools.configprocess import load_config, rewrite_config


def train_one(save_path, config, main_config, log_file_dir, index, logfile_level, console_level, device):
    """
    train an agent
    ...
    tradertrainer = TraderTrainer(config, main_config, save_path=save_path, device=device, index=index)
    asset_no = tradertrainer.coin_number()
    if asset_no != main_config["input"]["coin_number"]:
        rewrite_config(index=index, coin_num=asset_no)
        rewrite_config(coin_num=asset_no)
    return tradertrainer.train_net(log_file_dir=log_file_dir, index=index)
