...
from pgportfolio.tools.configprocess import rewrite_config
...
class TraderTrainer:
    def __init__(self, config, main_config, fake_data=False, restore_dir=None, save_path=None, device="cpu",
                 agent=None, index=None):
        """
    ...
    def train_net(self, log_file_dir="./tensorboard", index="0"):
        ...
        """
        self.__print_upperbound() # used self.test_set['y']
        if log_file_dir:
            if self.device == "cpu":
                with tf.device("/cpu:0"):
                    self.__init_tensor_board(log_file_dir)
            else:
                self.__init_tensor_board(log_file_dir) # used pv, log_mean, log, loss
        starttime = time.time()
    ...
    def __log_result_csv(self, index, time): # here the index is folder name
        ...
        tflearn.is_training(False, self._agent.session) # specifying layers defined by testing instead of training

        weights = self._evaluate("test", self._agent.portfolio_weights) # here (66,339)
        ...
        self._time_index_test = self._time_index[-int(len(self._time_index)*self._test_portion-self.__window_size):]
        weight_df = pd.DataFrame(weights[0], index = self._time_index_test, columns = self._asset_list)
