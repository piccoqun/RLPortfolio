...
import pgportfolio.marketdata.globaldatamatrix_new as gdm_new
import numpy as np
import pandas as pd
import logging
...
class DataMatrices:
    def __init__(self, start, end, period, batch_size=50, volume_average_days=30, buffer_bias_ratio=0,
        ...
        self._coin_no = coin_filter
        type_list = get_type_list(feature_number) #['close', 'high', 'low']
        self.__features = type_list
        self.feature_number = feature_number
        volume_forward = get_volume_forward(self.__end-start, test_portion, portion_reversed) # (end-start)*test_portion
        ...
        if market == "poloniex":
            ...
            self.__global_data = self.__history_manager.get_global_panel(start,
                                                                         ...
                                                                         period=period,
                                                                         features=type_list)
            self.__dates = self.__global_data.minor_axis.tolist()
        else:
            self.__history_manager = gdm_new.HistoryManager()
            self.__global_data = self.__history_manager.get_global_panel(start, self.__end, features=type_list)
            self._coins = self.__global_data.major_axis.tolist()
            self._coin_no = len(self._coins)
            self.__dates = self.__global_data.minor_axis.tolist()
        ...
        self.__PVM = self.__PVM.fillna(1.0 / self._coin_no)

        self._window_size = window_size
        self._num_periods = len(self.__global_data.minor_axis)
        self.__divide_data(test_portion, portion_reversed) # get train_ind, test_ind
    ...
    def next_batch(self):
        ...
        batch_size
        """
        # next_experience_batch: get start point randomly
    ...
    def __pack_samples(self, indexs):
        ...
        M = [self.get_submatrix(index) for index in indexs] # panel value with sub indexes of minor axis
        M = np.array(M)
        X = M[:, :, :, :-1]
        y = M[:, :, :, -1] / M[:, 0, None, :, -2]
