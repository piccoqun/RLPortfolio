...
def fill_input_default(input_config):
    ...
    set_missing(input_config, "is_permed", False) # random start of batch
...
def rewrite_config(index=None, coin_num = 11):
    if index:
        with open(rootpath+"/train_package/" + str(index) + "/net_config.json") as file:
            config = json.load(file)
        config['input']['coin_number'] = coin_num
        with open(rootpath+"/train_package/" + str(index) + "/net_config.json",'w') as file:
            json.dump(config, file)
    else:
        with open(rootpath+"/pgportfolio/" + "net_config.json") as file:
            config = json.load(file)
        config['input']['coin_number'] = coin_num
        with open(rootpath+"/pgportfolio/" + "/net_config.json",'w') as file:
            json.dump(config, file)
